...
# create User
@router.post('/user',status_code=status.HTTP_201_CREATED,response_model=schemas.ShowUserModel)
def create_user(request:schemas.UserModel,db:Session = Depends(get_db)):
    return user_repo.create_task(request,db)

#Get All User
@router.get('/users',response_model=List[schemas.ShowUserModel],status_code=status.HTTP_200_OK)
def all_user(db:Session = Depends(get_db)):
 
        return user_repo.all_user(db)

#Get User by ID
@router.get('/user/{id}',status_code=200,response_model=schemas.ShowUserModel)
